...
int usb_disconnect; /* if set to 0, main function will go through disconnect logic */
...
void TIMER0_IRQHandler(void) 
{
  ...
  uint32_t cnt;
	...
	while(usb_disconnect){
		select_watch = get_button();
		if(select_watch == KBD_SELECT) {
...
int mp3_player (void)
{
  ...
  GLCD_Bitmap (0, 30, 320, 222, (unsigned char*)GIMP_IMAGE_pixel_data);	
	GLCD_SetBackColor(White);
	GLCD_SetTextColor(Black);
	GLCD_DisplayString(0, 0, __FI, "   Potentiometer     ");							
	GLCD_DisplayString(1, 0, __FI, "   Adjusts Volume      ");										
	GLCD_DisplayString(9, 0, __FI, "   Select to Exit    ");
